...
def genSimTradingParams(cfg, title, lstParams: list, ignoreCache: bool = False):
    for i in range(len(lstParams)):
        yield libtrading2.tradingdb2_pb2.RequestSimTrading(
            basicRequest=libtrading2.trading2_pb2.BasicRequestData(
                token=cfg['token'],
            ),
            params=lstParams[i],
...
def simTradings(cfg, lstParams: list, ignoreCache: bool = False) -> list:
    ...
    stub = libtrading2.tradingdb2_pb2_grpc.TradingDB2Stub(channel)
